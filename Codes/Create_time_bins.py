...
from Create_classic_metrics import add_classic_metrics
...
def analyse_FED_file(df, inputs):
    ...
    # Find the original time bins. This table is retained unchanged for
    # the individual mouse workbook so it remains useful diagnostically.
    sheets["Time bins"] = find_time_bins(
        df,
        inputs,
    )

    # Build a separate copy for master aggregation. ClassicFED duration
    # standardisation and derived metrics apply only to this copy.
    master_time_bins = add_classic_metrics(
        sheets["Time bins"].copy(),
        df,
        inputs,
    )
    ...
    # Add date/time information to the master-only table after the
    # individual workbook has been exported, so this extra table is not
    # written as another worksheet in the individual output.
    sheets["Master time bins"] = add_additional_columns_to_sheet(
        master_time_bins,
        inputs,
    )
